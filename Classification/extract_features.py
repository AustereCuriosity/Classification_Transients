...
class Features():
    ...
    def __init__(self, series, **kwargs):
        ...
        outliers = kwargs.pop('outlier_remove', False)
    ...
    def outliers(self, potential_out):
                ...
                for i in range(0,len(potential_out)-1):
                    if(potential_out[i]+1 == potential_out[i+1]):
                        ...
                        if(pattern > 2):
                            ...
                            index = np.argwhere(potential_out == first)
                            two_pattern.append([index, index+1])
                        pattern = 1
                        first = None
        if(pattern > 2):
            index = np.argwhere(potential_out == first)
        ...
        if(len(two_pattern) >= 2):
            potential_out = np.delete(potential_out, flatten(indices)+flatten(two_pattern))
        else:
    ...
    def amplitude(self):
        ...
        self.features['amplitude'] = (np.median(sorted_mag[-int(math.ceil(0.01 * self.n)):]) - np.median(sorted_mag[0:int(math.ceil(0.01 * self.n))])) / 2.0   
    ...
    def Period(self):
        frequency, power = LombScargle(self.time, self.magnitude, self.mag_err).autopower(minimum_frequency=0.1, maximum_frequency=100)
        frequency = np.array(frequency)
        power = np.array(power)
        inds = power.argsort()
        self.features['period'] = int(frequency[inds][-1:])
...
if __name__=='__main__':
    ...
    series = pd.read_csv('sample_series/FILE1109065034047',header=None, sep=',')
    series.columns = ['Numerical_ID','MJD','Mag','Mag_err','RA','DEC']
    f = Features(series, mag_err=True, outlier_remove=True)
    f.call_all_features()
